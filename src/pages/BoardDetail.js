...
function BoardDetail({ user }) { //props user->현재 로그인한 사용자의 username
    ...
    const handleCommentEdit = (comment) => {
        ...
        setEditingCommentContent(comment.content);//수정할 내용으로 저장
    }
    ...
    if(!post) return <p style={{color:"blue"}}>해당 게시글이 존재하지 않습니다.</p>
    ...
    const isAuthor = user && user === post.author.username;
      ...
      return (
            ...
            {comments.length === 0 && <p style={{color:"blue"}}>등록된 댓글이 없습니다</p>}
            {comments.map((c)=>(
